...
## Adición en el sitio: términos técnicos
RNF-11 pide explicar los términos técnicos al pasar el cursor o al tocar, y el sistema no traía un componente para eso. El sitio usa `termino()` (`site/src/components/glosario.js`): el término queda en el texto con subrayado punteado en oro y, al pasar el cursor, tocarlo o activarlo con el teclado, abre una burbuja con la definición (fondo `--color-bg`, borde `--color-divider`, `--radius-md` y `--shadow-md`, como las sugerencias del buscador). La definición es también la descripción accesible del botón, se cierra con Escape y nunca se sale de la pantalla a 360 px. Las definiciones siguen las reglas de contenido: tuteo, frases cortas y sin signos de exclamación. No va dentro de tarjetas que son enlaces. Pendiente de aprobación del dueño del sistema de diseño (25 sep 2026).
